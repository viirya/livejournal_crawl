...
while ($stripped_result =~ s/<img.*?><a href=\'(.*?)\'><b>(.*?)<\/b><\/a>(.*)/$3/s) {
  my %blog;
  $blog{'url'} = $1;
  $blog{'user'} = $2;
  push @blogs, \%blog;
...
foreach (@blogs) {  
  ...
  while (1) {
    ...
    while ($comment_html =~ s/<a name=\'(.*?)\'><\/a><table.*?class=\'talk-comment\'>(.*?)<\/table>(.*)/$3/s) {
      my $comment_content = $2;
      my $comment_id = $1;

      $comment_content =~ s/<span class=\'ljuser\'.*?>.*?<\/a><a href=\'(.*?)\'><b>(.*?)<\/b><\/a>(.*)/$3/s;
      print $1 . "   " . $2 . "\n";
      
      my $commentor_blog_url = $1;
      my $commentor = $2;
      ...
      # obtain keywords
      if ($comment_content =~ m/link<\/a>\).*?<td>(.*?)\(<a.*?>Reply to this<\/a>\)/s) {
        my $comment_text = $1;
        print "comment text: " . $1 . "\n";
      ...
      if ($comment_content =~ m/Reply to this<\/a>\)\(<a href=\'(.*?)\#(.*?)\'>Parent<\/a>/s) {
        my $comment_reply_parent = $2;
